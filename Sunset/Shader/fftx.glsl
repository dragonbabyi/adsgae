...
vec4 fft2(int layer, vec2 i, vec2 w) {
    vec4 input1 = textureLod(imgSampler, vec3(i.x, uv.y, layer), 0.0);
    vec4 input2 = textureLod(imgSampler, vec3(i.y, uv.y, layer), 0.0);
...
void main() {
    vec4 data = textureLod(butterflySampler, vec2(uv.x, pass), 0.0);
    vec2 i = data.xy;
    vec2 w = data.zw;
